...
async function main() {
    ...
    for (const entry of dataWithEmbeddings) {
        ...
        const dotProductValue = dotProduct(entry.embedding, inputEmbedding.data[0].embedding);
        similarities.push({
            input: entry.input,
            similarity,
            dotProductValue
    ...
    sortedSimilarities.forEach(similarity => {
        console.log(`${similarity.input}: ${similarity.similarity} (dot product: ${similarity.dotProductValue})`);
    })
}
